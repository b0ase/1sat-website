...
import { FaSort, FaSortUp, FaSortDown, FaHashtag, FaShareAlt } from "react-icons/fa";
			...
			<tr className="text-sm font-semibold">
				<th className={`min-w-16 ${sortable ? "cursor-pointer" : ""} group flex items-center`}>
					{type === AssetType.BSV20 && (
						...
						<div className="mr-2 hover:text-white text-xs" onClick={() => handleSort(SortBy.Number)}>
							<FaHashtag />
						</div>
					)}
					{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
				...
				<th className="w-16 text-center">
					<div className="flex items-center justify-center">
						<FaShareAlt className="text-[#555] text-xs" />
					</div>
				</th>
				{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
				<th
				...
				>
					<div className="flex items-center">
						Recent Price
						{sortable && <span
							className={`w-6 ml-1 md:invisible md:group-hover:visible ${sort === SortBy.Price ? "" : "text-[#555]"} text-xs`}
						>
							{sort === SortBy.Price ? (
								dir === "asc" ? (
									<FaSortUp />
				...
				>
					<div className="flex items-center">
						Pct Change
						{sortable && <span
							className={`w-6 ml-1 md:invisible md:group-hover:visible ${sort === SortBy.PctChange ? "" : "text-[#555]"} text-xs`}
						>
				...
				>
					<div className="flex items-center justify-end">
						Market Cap
						{sortable && <span
							className={`w-6 ml-1 md:invisible md:group-hover:visible ${sort === SortBy.MarketCap ? "" : "text-[#555]"} text-xs`}
						>
				...
				>
					<div className="flex items-center justify-end">
						Holders
						{sortable && <span
							className={`w-6 ml-1 md:invisible md:group-hover:visible ${sort === SortBy.Holders ? "" : "text-[#555]"} text-xs`}
						>
							{sort === SortBy.Holders ? (
								dir === "asc" ? (
									<FaSortUp />
