...
import SocialLinksManager from "@/components/SocialLinks/SocialLinksManager";
import { toastErrorProps, type AssetType } from "@/constants";
import { bsv20Balances, ordAddress } from "@/signals/wallet";
...
export function TokenMarketTabs({
  ...
  // Check if current user is the token creator (simplified check)
  const isCreator = computed(() => {
    // This is a simplified check - in a real app you'd want to verify ownership properly
    return Boolean(ordAddress.value && ticker.data?.insc?.json?.creator === ordAddress.value);
  });
  ...
  return ticker && (
    <div className="flex flex-col w-full">
      ...
      <div className="mb-6 p-4 bg-base-100 rounded-lg border border-base-300">
        <SocialLinksManager
          tokenId={ticker.id || ticker.tick || ''}
          tokenSymbol={ticker.sym || ticker.tick || ''}
          currentSocialLinks={ticker.socialLinks}
          isCreator={isCreator.value}
          onUpdate={(socialLinks) => {
            // Update the ticker data with new social links
            if (ticker) {
